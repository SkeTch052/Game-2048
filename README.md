...
- Qt: Версия 5.15.2 или новее (протестировано с MSVC2019_64);
- CMake: Версия 3.16 или новее;
- Компилятор C++: MSVC 2019 или совместимый (для Windows);
...
Главное меню:\
Нажмите "Начать", чтобы запустить игру (4x4 или 5x5, в зависимости от выбранного режима).\
Нажмите "Рекорды", чтобы просмотреть таблицу лучших результатов.\
Нажмите "Выход", чтобы закрыть приложение.


Игровой процесс:\
Используйте стрелки на клавиатуре для перемещения плиток и объединения одинаковых чисел.\
Достигните плитки 2048, чтобы выиграть (появится диалог победы).\
Игра завершается, если ходы невозможны (появится диалог окончания).
...
- Qt 5.15.2 или новее: Основной фреймворк для UI и логики.
- CMake: Для сборки проекта.
- C++11: Используется для современных конструкций.
...
```
cd build\Release
<Path to Qt>\6.4.2\mingw_64\binwindeployqt Game2048.exe
```
